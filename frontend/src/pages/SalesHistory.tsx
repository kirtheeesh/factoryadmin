            ...
            <header className="flex flex-col md:flex-row md:items-end justify-between gap-6 pb-6 border-b border-slate-100">
                <div className="space-y-1">
                    <div className="flex items-center gap-2 text-[#e85c24] text-xs font-black uppercase tracking-[0.2em] mb-2">
                        <TrendingUp size={14} /> Revenue Records
                    </div>
                    <h1 className="text-4xl font-black text-slate-900 tracking-tight">Sales History</h1>
                    <p className="text-slate-500 font-medium">Review and download all approved sales invoices and batch records.</p>
                </div>
            </header>
                    ...
                    <table className="w-full text-left border-collapse">
                        <thead>
                            <tr className="bg-slate-50/50 border-b border-slate-100">
                                <th className="px-8 py-6 text-[10px] font-black text-slate-400 uppercase tracking-widest">Invoice Details</th>
                                <th className="px-8 py-6 text-[10px] font-black text-slate-400 uppercase tracking-widest text-center">Batch Number</th>
                                <th className="px-8 py-6 text-[10px] font-black text-slate-400 uppercase tracking-widest text-center">Total Amount</th>
                            ...
                            {loading ? (
                                ...
                                <tr>
                                    <td colSpan={6} className="px-8 py-20 text-center">
                                        ...
                                        <p className="text-slate-400 font-bold">No sales history found</p>
                                    </td>
                                </tr>
                            ) : (
                                    ...
                                    >
                                        <td className="px-8 py-6">
                                            <div className="flex items-center gap-4">
                                                ...
                                                <div>
                                                    <p className="text-sm font-black text-slate-900 uppercase tracking-tight">INV #{item.invoice_number}</p>
                                                    <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mt-0.5">{new Date(item.created_at).toLocaleDateString()}</p>
                                                </div>
                                            </div>
                                        </td>
                                        ...
                                        <td className="px-8 py-6 text-center">
                                            <span className="text-sm font-black text-emerald-600">₹{parseFloat(item.total_amount).toFixed(2)}</span>
                                        </td>
                                        <td className="px-8 py-6">
                                            <span className="text-[10px] font-black text-slate-500 uppercase tracking-widest">{item.customer_name || item.customer_name_manual}</span>
                                        </td>
                                        <td className="px-8 py-6">
